...
export function normalizePartitions(
  ...
  const normalized = partitions.map((partition) => {
    ...
    const subtype =
      typeof partition.subtype === "string"
  ...
  for (const part of normalized) {
    const isPrimaryBootloader =
      part.type === BOOTLOADER_TYPE && part.subtype === SUBTYPES[BOOTLOADER_TYPE].primary;
    ...
    if (part.offset !== null && part.offset < lastEnd) {
      throw new Error(
        `Partitions overlap: partition ${part.name} sets offset 0x${part.offset.toString(16)}, previous end is 0x${lastEnd.toString(16)}`,
      );
    }
...
export function verifyPartitions(
  ...
  for (const part of sortedByOffset) {
    if (part.offset < options.offset + PARTITION_TABLE_SIZE) {
      const isPrimaryBootloader =
        part.type === BOOTLOADER_TYPE && part.subtype === SUBTYPES[BOOTLOADER_TYPE].primary;
      const isPrimaryPartitionTable =
        part.type === PARTITION_TABLE_TYPE &&
        part.subtype === SUBTYPES[PARTITION_TABLE_TYPE].primary;
      if (!isPrimaryBootloader && !isPrimaryPartitionTable) {
        throw new Error(
          `Partition offset 0x${part.offset.toString(16)} is below 0x${(options.offset + PARTITION_TABLE_SIZE).toString(16)}`,
        );
      }
    }
  ...
  const otadata = partitions.filter(
    (x) => x.type === DATA_TYPE && x.subtype === SUBTYPES[DATA_TYPE].ota,
  );
  if (otadata.length > 1) {
  ...
  const teeOtadata = partitions.filter(
    (x) => x.type === DATA_TYPE && x.subtype === SUBTYPES[DATA_TYPE].tee_ota,
  );
  ...
  if (options.flashSizeMb !== undefined) {
    ...
    if (end > flashSize) {
      throw new Error(
        `Partition table occupies ${end} bytes and does not fit in ${options.flashSizeMb}MB flash`,
      );
    }
  }
}
...
function verifyPartition(partition: NormalizedPartition, options: NormalizedOptions): void {
  ...
  if (partition.offset % offsetAlign !== 0) {
    throw new Error(
      `Partition ${partition.name} offset 0x${partition.offset.toString(16)} is not aligned to 0x${offsetAlign.toString(16)}`,
    );
  }
  ...
  if (partition.type === APP_TYPE) {
    ...
    if (partition.size % sizeAlign !== 0) {
      throw new Error(
        `Partition ${partition.name} size 0x${partition.size.toString(16)} is not aligned to 0x${sizeAlign.toString(16)}`,
      );
    }
  }
  ...
  const alwaysRw =
    ...
    (partition.subtype === SUBTYPES[DATA_TYPE].ota ||
      partition.subtype === SUBTYPES[DATA_TYPE].coredump);
  ...
  if (partition.type === DATA_TYPE && partition.subtype === SUBTYPES[DATA_TYPE].nvs) {
    if (partition.size < NVS_RW_MIN_PARTITION_SIZE && !readonly) {
      throw new Error(
        `Partition ${partition.name} NVS size 0x${partition.size.toString(16)} requires readonly flag`,
      );
    }
  }
}
